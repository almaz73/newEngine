  ...
  useDesktopStore().getAutoVIN(vin).then(res => {
    if (res.autoId) ElMessageBox.confirm('Автомобиль с текущим VIN существует. Заменить?', '', {
      ...
      if (yes) {
        ...
        auto.value.carModelId = res.carModelId
        auto.value.yearReleased = new Date(res.yearReleased)
      }
...
function chanheModel(modelId) {
  useAdminStore().getGenerations(modelId).then(res => {
    carGenerations.value = res
  })
}
