...
import { CategoryAuto, SteeringWheelType, bodyTypes } from '@/utils/globalConstants'
...
function save() {
  checkEmptyFields(form.value).then((res:any) => {
    console.log('res = ',res)
    console.log('newDeal.value = ',newDeal.value)
...
function open(appeal) {
  ...
  console.log('>>>auto = ',appeal)

  isOpen.value = true

  newDeal.value.auto = appeal.auto

  console.log('????nappeal.mileageAuto = ',appeal.mileageAuto)


  newDeal.value.mileageAuto = appeal.mileageAuto
