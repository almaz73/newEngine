<template>
  <div id="captchaDialog"
       v-if="isOpen"
       style="margin-top: -400px;left: 125px;
         position: absolute;
         z-index: 9999;
         width: 180px;
         padding: 15px;
         box-shadow: 5px 5px 10px 1px #333333;
         border-radius: 7px;
         background: #ddd;
         text-align: center;">
    <div>Введите цифры</div>
    <div style="cursor: pointer;" v-if="gibdd.captcha">
      <img :src="'data:image/png;base64,'+gibdd.captcha"
           alt="обновить капчу.."
           style="min-height: 60px; background: #bbb"
           @click="getCaptcha()" />
    </div>

    <div style="cursor: pointer;" v-if="!gibdd.captcha">
      <img style="min-height: 60px; width: 173px"
           @click="getCaptcha()"
           src="data:image/gif;base64, ">
    </div>

    <div v-if="!gibdd.captcha">
      загрузка капчи..
    </div>

    <div>
      <input v-model="gibdd.captcha_num"
             autocomplete="off"
             @input="inpChanged()"
             v-focus
             style=" padding: 7px 7px;
                       font-size: 24pt;
                       width: 163px;
                       text-align: center;
                       margin: 5px 0;
                       border: 1px solid #999;">
    </div>

    <div id="captchaButtons">
      <button @click="closeModal(), getCapcha('upd')">Обновить</button>
      <button @click="closeModal()">Отменить</button>
    </div>
  </div>

</template>


<script setup>
import { ref } from 'vue'
import { useGlobalStore } from '@/stores/globalStore'


let cb = null
let getCapcha = null
const globalStore = useGlobalStore()
const isOpen = ref(false)
const closeModal = () => isOpen.value = false
const { gibdd } = defineProps(['gibdd'])


function open(cbModal, getCapcha_) {
  cb = cbModal
  getCapcha = getCapcha_
  isOpen.value = true
}


function inpChanged() {
  if (gibdd.captcha_num.length === 5) {
    closeModal()
    cb()
  }
}

defineExpose({ open })


</script>