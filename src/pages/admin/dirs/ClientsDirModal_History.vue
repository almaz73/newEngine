<template>
  <AppModal v-if="isOpen"
            ...
            :top="50"
            :title="'История изменений клиента'"
            :subtitle="subtitle"
            draggable
            resizable>
...
function open(client) {
  ...
  isOpen.value = true
  subtitle.value = ''
  if (client.person.firstName) subtitle.value += client.person.firstName + ' '
  if (client.person.middleName) subtitle.value += client.person.middleName + ' '
  if (client.person.lastName) subtitle.value += client.person.lastName + ' '

  adminStore.getClientHistory(client.leadId).then(res =>tableData.value = res)
}
